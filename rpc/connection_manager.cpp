...
bool ConnectionManager::Initialize() {
    ...
    if (setrlimit(RLIMIT_NOFILE, &rt) == -1) {
        LOGERROR("set setrlimit falied size %d\n", MAXEPOLLSIZE);
        return false;
    }
    return true;
}
...
int32_t ConnectionManager::TcpListen(
    ...
    if ((getaddrinfo(set_host_, set_port_, &hints, &res)) != 0) {  
        LOGERROR("tcp_connect error for %s, %s\n", set_host_, set_port_);
        return -1;
    }
    ...
    do {
        ...
        if (setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) < 0) {
            LOGERROR("setsockopt error listenfd %s\n", listenfd);
            return -1;
        }
    ...
    if (listen(listenfd, 20/*LISTENQ*/) < 0) {
        LOGERROR("listen error listenfd is %d\n", listenfd);
        return -1;
    }
...
int32_t ConnectionManager::EpollInit(int32_t listenfd) {
    ...
    if (epoll_ctl(ep_create_fd_, EPOLL_CTL_ADD, listenfd, &ev_) < 0) {
        LOGERROR("epoll set insertion error: fd = %u\n", listenfd);
        return -1;
    }
    return 0;
}
...
int32_t ConnectionManager::EpollWait(int32_t max_events, struct epoll_event* events) {
    ...
    if (ready_nfds < 0) {
        LOGERROR("epoll wait error return fd is %d\n", ready_nfds);
        return -1;
    }
    return ready_nfds;
...
bool ConnectionManager::SetNonBlock(int32_t sock) {
    ...
    if (opts < 0) {
        LOGERROR("set non block failed! fcntl(sock,GETFL)\n");
        return false;
    }
...
bool ConnectionManager::ConfigSet(const char* host, const char* port) {
    ...
    if (conf_host_size > SOCKINFO_MAXSIZE || param_host_size > SOCKINFO_MAXSIZE) {
        LOGERROR("invalid host  conf host: %s, param host: %s\n",  conf_host_, host);
        return false;
    }
    if (conf_port_size > SOCKINFO_MAXSIZE || param_port_size > SOCKINFO_MAXSIZE) {
        LOGERROR("invalid port  conf port: %s, param port: %s\n", conf_port_, port);
        return false;
    }
